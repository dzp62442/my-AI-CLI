...
默认列出排队中、部署中、运行中、停止中的任务。处理用户任务时加名称过滤，并显式指定输出格式：

```bash
. "$HOME/.volc_env"
volc ml_task list --name dzp --output json
```

`--output json` 下默认只返回 10 条。需要更多历史任务时显式使用 `--limit` 和 `--offset`：

```bash
volc ml_task list -n dzp --limit 100 --offset 0 --output json
```

按状态筛选：

```bash
volc ml_task list -n dzp --status Queue,Staging,Running --output json
volc ml_task list -n dzp --status Success,Failed,Killed,Exception --output json
```
...
实例 ID 常见形态如 `worker_0`。看日志前先确认实例 ID。
